...
user_hilang_bp = Blueprint('user_hilang', __name__, url_prefix='/hilang')
...
# -------------------------------
# Halaman Form Surat Hilang untuk User
# -------------------------------
@user_hilang_bp.route('/form', methods=['GET', 'POST'])
def form_hilang():
    conn = get_connection()
    with conn.cursor() as cursor:
        ...
        if request.method == 'POST':
            ...
            return redirect(url_for('user_hilang.form_hilang'))

    conn.close()
    return render_template('user/hilang_form.html', now=datetime.now, jumlah_kunjungan=jumlah_kunjungan)


# -------------------------------
# API: Ambil Data Otomatis dari KK & NIK
# -------------------------------
@user_hilang_bp.route('/ambil-data')
